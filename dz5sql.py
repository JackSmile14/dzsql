...
def delete_tables(cur):
    """Удаляет созданные данные для отладки"""
    cur.execute("""
...
def add_client(cur, client_id, first_name, last_name, email, *phones):
    """Добавляет все данные клиента в таблицу Clients, а телефоны в Client_phones"""
    cur.execute("""
...
with psycopg2.connect(database="psycopg_db", user="postgres", password="234titan7865") as conn:
    with conn.cursor() as cur:
        delete_tables(cur)
        create_db(cur)
        add_client(cur, 1, 'Any', 'Anybody', 'any@mail.foo', '+123-123', '+375-987')  #Добавляем клиента с телефонами
        add_client(cur, 2, 'Sam', 'Sambody', 'some@mail.foo')  #Добавляем клиента без телефона
        add_client(cur, 3, 'Jack', 'Nobody', 'no@mail.foo', '+000-000', '+111-111')
        add_phone(cur, 2, '+375-111')  #Добаляем телефон клиенту
        ...
        find_client(cur, phone_number='+375-011')
